(function () {
	...
	const $btnToggle_arr = $('.btn-toggle');


	$btnToggle_arr.each(function (i, el) {
		...
		const elDataSwipe = $btnToggle.data('toggle-swipe') || false;
		const elDataSwipeDirection = $btnToggle.data('toggle-swipe-direction') || 'left';
		...
		const handlerToggleClick = function (e) {
			...
			const handlerOverflowClick = function (e) {
				...
				if (check) {
					...
					$(document).off('mouseup', handlerOverflowClick);
				}
			};
			...
			if ($ToggleTarget.length > 0) {
				...
				if ($btnToggle.hasClass('active')) {
					...
					if (dataOverflowClick && windowsWidth > dataOverflowClickMinWidth) {
						$(document).on('mouseup', handlerOverflowClick);
					}
				} else {
					$ToggleTarget.removeClass('active');
		...
		const btnToggleInit = function () {
			if (btnToggle.length) {
				btnToggle.off('click', handlerToggleClick);
				btnToggle.on('click', handlerToggleClick);
			}
		};
		const handlerToggleSwipe = function () {
			const handlerOverflowClick = function (e) {
				const $target = $(e.target);
				const check = !$target.is($ToggleTarget) && !$target.is($btnToggle) && !$target.closest($ToggleTarget).length > 0 && !$target.closest($btnToggle).length > 0;
				if (check) {
			...
			if ( elDataSwipe ) {
				$ToggleTarget.swipe({
					swipe:function(event, direction, distance, duration, fingerCount, fingerData) {
						if ( direction === elDataSwipeDirection ) {
							$btnToggleCloned.each(function (i, el) {
								let data = $(el).data('body-overflow');
								if (data) {
									$ToggleBodyOverflowCloned = true;
