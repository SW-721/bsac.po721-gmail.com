...
;(function (factory) {
	...
	Slick.prototype.lazy = function (state) {
		var _ = this;
		...
		var callback_loaded = function (element) {
			...
			_.$slider.trigger('imgLazyLoaded', [_, element]);
		};
		...
		if (!window.LazyLoad) {
			...
			if (_state) {
				var $img_active = _.$slider.find('.slick-active img, .slick-active .lazy-bg-slider');
				var img_active_lazy_load = new LazyLoad({
					callback_loaded: callback_loaded
				}, $img_active);
			} else {
				var $img_arr = _.$slider.find('img, .lazy-bg-slider');

				$img_arr.each(function (i, el) {
					var $img = $(el);
	...
	$.fn.slick = function () {
		...
		for (i = 0; i < l; i++) {
			if (typeof opt == 'object' || typeof opt == 'undefined') {
				var _i = i
				if (typeof opt.lazy_init == 'undefined') {
					if (!window.Waypoint) {
						...
						var waypoint = new Waypoint({
							...
							handler: function (direction) {
								_[_i].slick = new Slick(_[_i], opt);
								waypoint.destroy();
							},
							offset: opt.lazy_init_offset || 400
						})
					}
				} else {
					_[i].slick = new Slick(_[i], opt);
